...
namespace RayTracer
{
    class Cones: public AShape {
        ...
        hits(const Ray &ray, Math::RGBA &hitColor, double &t) const override
        {
            ...
            double b = 2 * (dot_ray_vect * (ptToRay.dot(m_vect)) - (ray.m_direction.dot(ptToRay)) * cosPow);
            double c = ((ptToRay.dot(m_vect)) * (ptToRay.dot(m_vect))) - ptToRay.dot(ptToRay) * cosPow;
        ...
        {
            auto ptToOrigin = pt.getVectorTo(m_point);
